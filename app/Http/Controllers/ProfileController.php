...
class ProfileController extends Controller
{
    ...
    public function saveHobbies(Request $request, $username){
        ...
        $request->session()->flash('alert-success', 'Your Interests have been successfully updated!');

        return redirect('/'.Auth::user()->username);
        }else{
         
        $request->session()->flash('alert-danger', 'there is a problem');    
    ...
    public function uploadProfilePhoto(Request $request, $username){
        ...
        if ($validator->fails()) {
            ...
            if ($file->storeAs('public/uploads/profile_photos', $file_name)){
                ...
                $response['code'] = 400;
                $response['message'] = "Image is too large!";
            }
        }
        return Response::json($response);
    ...
    public function flag($id = null){
       ...
       $alu = DB::table('Alerts')->where('alid', $user)->where('aluid',$id)->get()->first();

       if($alu == null ){
           
          
        DB::table('Alerts')->insert(['alid' => $user, 'aluid'=> $id]);
        DB::table('users')->where('id', '=', $id)->increment('rate');
